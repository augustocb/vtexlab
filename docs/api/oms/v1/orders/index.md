...
| Recurso       | Descrição     |
| ------------- |--------------:|
| [GET /api/oms/pvt/orders](#listar-pedidos)  | Listar pedidos|
| [GET /api/oms/pvt/orders/{orderId}](#consultar-pedido)  | Consultar pedido|
| [POST /api/oms/pvt/orders/{orderId}/start-handling](#iniciar-manuseio)  | Iniciar manuseio|
| [POST /api/oms/pvt/orders/{orderId}/invoice](#notificar-faturamento)  | Notificar Faturamento|
| [POST /api/oms/pvt/orders/{orderId}/cancel](#cancelar-pedido)  | Cancelar pedido|
| [POST /api/oms/pvt/orders/{orderId}/changes](#registrar-mudanças)  | Registrar mudanças|

##Listar pedidos
Permite consultar uma lista de pedidos de acordo com os filtros informados.

###Parâmetros de Solicitação:
Todo os parâmetros são opcionais, exceto o "an" que pode ser substituido pelo host. Conforme o padrão de
URLs, todos os parâmetros são separados usando o caractere "e" comercial (&). Veja abaixo a lista de parâmetros e seus
valores possíveis.

Parâmetros:
>
- <b>`f_status:`</b> string que define um status de pedido. Caso seja passado, só serão retornados pedidos nesse status. Ver
na documentação anexa a lista dos possíveis status de pedido no seu respectivo workflow. Ex.:
f_status=ready-for-handling
- <b>`clientEmail:`</b> string com o valor do e-mail do cliente que efetuou a compra. Só serão retornados pedidos que possuem
esse e-mail (customer e-mail). Ex.: clientEmail=fred.santos@bol.com.br
- <b>`clientDoc:`</b> string com o valor do documento do cliente que efetuou a compra. Só serão retornados pedidos que
possuem esse número de documento (no Brasil é o número do CPF). Ex.: clientDoc=11111111111
- <b>`clientCorporateDoc:`</b> string com o valor do documento do cliente corporativo que efetuou a compra. Só serão
retornados pedidos que possuem esse número de documento (no Brasil é o número do CNPJ). Ex.:
clientCorporateDoc=11111111111001
- <b>`per_page:`</b> string com o valor número da quantidade de itens que se deseja obter (o default é 15). Ex.: per_page=30
- <b>`page:`</b> string com o valor número da página que se deseja obter. Ex.: page=3
- <b>`orderBy:`</b> string com o valor do campo e tipo de ordernação. Ex.: orderBy=creationDate,desc

####URL do Recurso:
```http
GET /api/oms/pvt/orders HTTP/1.1
```
...
####URL do Recurso:
```http
GET /api/oms/pvt/orders/{orderId} HTTP/1.1
```
...
####URL do Recurso:
```http
POST /api/oms/pvt/orders/{orderId}/start-handling HTTP/1.1
```
...
####URL do Recurso:
```http
POST /api/oms/pvt/orders/{orderId}/invoice HTTP/1.1
```
...
```json
{
  "date": "2014-02-06T22:46:48.9896369Z",
  "orderId": "359722648932-01",
  "receipt": "5d749bae-3c97-416f-8c58-a86323567438"
}
```
####Status codes: `200 OK` `404 NotFound` 
...
####URL do Recurso:
```http
POST /api/oms/pvt/orders/{orderId}/cancel HTTP/1.1
```
...
{
  "orderId": "359722648932-01",
  "receipt": "bebe199b-e418-4157-995a-e38459da1467",
  "date": "2014-02-06T22:46:48.9826365Z"
}
```
####Status codes: `200 OK` `404 NotFound` 
...
####URL do Recurso:
```http
POST /api/oms/pvt/orders/{orderId}/changes HTTP/1.1
```

####Exemplo de requisição:
```json
{
  "reason": "Reason0b439201-9aff-4452-8ab7-d87e4cab6241",
  "discountValue": 24,
  "itemsRemoved": [
    {
      "id": "Id1e212912-7890-4f24-a457-9f084958a329",
      "quantity": 101,
      "price": 82
    },
    {
      "id": "Id886fec63-a9ed-4dbd-8c0e-726cb303fa24",
      "quantity": 52,
      "price": 70
    },
    {
      "id": "Idf2ca380c-c156-478d-a176-937acba75d23",
      "quantity": 150,
      "price": 139
    }
  ],
  "itemsAdded": [
    {
      "id": "Id822d194e-1564-4e05-bb00-1aac8926fae6",
      "quantity": 123,
      "price": 184
    },
    {
      "id": "Id147233f3-af6e-4d04-8826-585a5da01237",
      "quantity": 61,
      "price": 85
    },
    {
      "id": "Id5d127009-b464-4957-bb8c-22ca0240c60f",
      "quantity": 51,
      "price": 14
    }
  ]
...
```json
{
  "date": "2015-10-11T07:11:40.0136419",
  "orderId": "OrderIdba629ea2-ef6a-4cbc-9788-11dc48062b4a",
  "receipt": "Receipt7dacb1d6-84fc-478c-a704-4bfd662e6021"
}
```
####Status codes: `200 OK` `404 NotFound` 
